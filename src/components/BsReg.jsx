...
import { BookOpen, CheckCircle, Loader2, MapPin, Home, Users, User, Phone, Calendar } from "lucide-react";
...
const BS_STRUCTURE = {
  "In Campus": {
    "Nairobi": "Mt Moriah A", "Eldoret": "Mt Moriah A",
    "Mombasa": "Mt Moriah B", "Argentina": "Mt Moriah B",
    "CBD": "Mt Pisgah A", "Mara": "Mt Pisgah A",
    "Maringo": "Mt Pisgah B", "Ruwenzori": "Mt Pisgah B",
    "Uganda": "Gilead A", "Barret": "Gilead A", "Old Hall": "Gilead A",
    "Taifa": "Gilead B", "Mama Ngina": "Gilead B",
    "Hollywood": "Bethany A", "Buruburu Block B": "Bethany A",
    "River View": "Bethany B", "Buruburu Block A": "Bethany B",
  },
  "Diaspora": {
    "Sumkam & Surrounding": "Jerusalem A", "Rubis & Surrounding": "Jerusalem B",
    "420": "Gethsemane A", "Booster (Diaspora)": "Gethsemane A",
    "Herbage & Surrounding": "Gethsemane B",
    "Ng'ondu & Surrounding": "Mt Olives A", "Wright & Surrounding": "Mt Olives B",
  },
  "Njokerio": {
    "Grace Hill": "Eden A", "Mzalendo & Surrounding": "Judea A",
    "Brothers & Surrounding": "Judea B", "Ngumu Industrial Area": "Zion A",
...
function BsReg() {
  const [formData, setFormData] = useState({
    fullName: "", phoneNumber: "", gender: "Male", yearOfStudy: "1.1",
    region: "", catchmentArea: "", groupName: ""
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(true);
      setFormData({ fullName: "", phoneNumber: "", gender: "Male", yearOfStudy: "1.1", region: "", catchmentArea: "", groupName: "" });
    } catch (err) {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Register for Bible Study groups at Egerton University SDA Church. Find your spiritual family based on your residence." />
      </Helmet>
      ...
      <div className="min-h-screen bg-gray-50 flex justify-center items-center md:py-8 px-4">
        ...
        <div className="bg-white max-w-2xl w-full rounded-2xl shadow-xl p-8 border-t-4 border-green-700 animate-in fade-in zoom-in duration-300">
          ...
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold text-gray-900">Bible Study Registration</h2>
            <p className="text-gray-500 mt-2">Join a family of believers near you.</p>
          </div>
          ...
          {success ? (
            <div className="text-center py-10 bg-green-50 rounded-xl border border-green-100">
              ...
              <h3 className="text-2xl font-bold text-green-800">Welcome Home!</h3>
              <p className="text-gray-600 mt-2 max-w-sm mx-auto">
                You have successfully registered. You belong to the <span className="font-bold text-green-900">{formData.groupName}</span> family.
              </p>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="bg-gray-50 p-5 rounded-xl border border-gray-100 space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                   ...
                   <div className="relative">
                      ...
                      <select 
                        name="yearOfStudy" value={formData.yearOfStudy} onChange={handleChange} 
                      ...
                      >
                        {["1.1", "1.2", "2.1", "2.2", "3.1", "3.2", "4.1", "4.2", "5.1", "5.2"].map(y => <option key={y} value={y}>Year {y}</option>)}
                      </select>
                   </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">Location & Group</h3>

                {/* 1. Region */}
                <div className={`transition-all duration-300 ${formData.groupName ? 'opacity-50' : 'opacity-100'}`}>
                  <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-1">
                      <MapPin size={16} className="text-green-600" /> Select Region
                  </label>
                  <select 
                    name="region" value={formData.region} onChange={handleChange} required 
                    className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 outline-none bg-white"
                  >
                      <option value="">-- Choose Region --</option>
                      {Object.keys(BS_STRUCTURE).map(region => <option key={region} value={region}>{region}</option>)}
                  </select>
                </div>
                ...
                {formData.region && (
                  <div className="animate-in slide-in-from-top-2">
                    <label className="flex items-center gap-2 text-sm font-bold text-gray-700 mb-1">
                      <Home size={16} className="text-green-600" /> Select Residence
                    </label>
                    <select 
                        name="catchmentArea" value={formData.catchmentArea} onChange={handleChange} required 
                        className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 outline-none bg-white"
                    >
                      <option value="">-- Choose Residence --</option>
                      {Object.keys(BS_STRUCTURE[formData.region]).map(area => <option key={area} value={area}>{area}</option>)}
                    </select>
                  </div>
                )}
                ...
                {formData.groupName && (
                  <div className="animate-in zoom-in duration-300 mt-4">
                    <div className="p-4 bg-green-50 border border-green-200 rounded-xl flex items-center justify-between shadow-sm">
                        <div>
                            <p className="text-xs text-green-600 font-bold uppercase mb-1">You belong to</p>
